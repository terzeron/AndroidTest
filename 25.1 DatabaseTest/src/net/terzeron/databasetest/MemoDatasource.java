...
public class MemoDatasource {
	...
	public Memo createMemo(String memo) {
		...
		try {
			...
			long insertId = database.insert(MySQLiteHelper.TABLE_MEMOS, null,
					values);
			cursor = database.query(MySQLiteHelper.TABLE_MEMOS, allColumns,
	...
	public void deleteMemo(Memo memo) {
		long id = memo.getId();
		database.delete(MySQLiteHelper.TABLE_MEMOS, MySQLiteHelper.COLUMN_ID
				+ " = " + id, null);
	}
	...
	private Memo cursorToMemo(Cursor cursor) {
		...
		int memoIndex = cursor.getColumnIndex(MySQLiteHelper.COLUMN_MEMO);
